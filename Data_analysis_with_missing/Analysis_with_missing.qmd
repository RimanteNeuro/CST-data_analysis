---
title: Demographic table, linear regression and mediation anlysis with missing data points 
...
```{r}
women_data_drop <- women_data |> drop_na( EA1, EA2, Tiredness_1, Tiredness_2, P4, E2, Tes)
```
...
```{r}
women_lm_model <- lm(accu~ OC + IUD + NCL + Age + STEM + PANAS_PA + PANAS_NA + EA1 + EA2 + Tiredness_1 + Tiredness_2 + Femininity + Masculinity + P4 + E2 +Tes, data = women_data_drop)
```
...
```{r}
women_final <- lm(accu ~ OC + NCF + NCL + STEM + EA2 + Tes, data  = women_data_drop)
```
...
global_model <- lm(accu~ OC + IUD + NCL + Age + STEM + PANAS_PA + PANAS_NA + EA1 + EA2 + Tiredness_1 + Tiredness_2 + Femininity + Masculinity + P4 + E2 +Tes, data = women_data, na.action=na.fail)

# Perform model selection
model_set <- dredge(global_model)

# Model averaging
averaged_model <- model.avg(model_set, subset = delta < 2)

# AIC of the averaged model
AIC(averaged_model)
```
...
The independence of error assumption is notviolated.
...
```{r}
#| eval: false
process (data = CST_full_parameters, y = "accu", x = "Sex", m ="EA2", model = 4, effsize =1, total =1, stand =1, cov = c("Age", "STEM"), boot = 10000 , seed = 654321)
```
Percentage of the sex difference explained by EA2 


```{r}
Proc_EA <- (0.1361*100)/0.6144
Proc_EA
```
