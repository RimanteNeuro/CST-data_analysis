...
```{r}
#| echo: false
combined_full <- read_csv("C:/Users/Rokas/Desktop/Mantes dokumentai/Dok. reikalingi failai/Skerspjūvio analizė/final_final_analysis/Full_CST_analysis/combined_full.csv")
```
...
Violated just in males group, probably due to sealing effect. In men group n \> 30, still can use parametric statistic (central limit theorem).

**Homoscedasticity test**

```{r}
model <- lm(accu ~ Group + STEM, data = CST_full_parameters)
bptest(model)
```

Homoscedasticity assumption is not violated p \> 0.05

**Conclusion:** Assumptions are not violated, we can use parametric statistic.
...
```{r}
women_final <- lm(accu ~ OC + NCF + NCL + EA2 + Femininity + Masculinity, data  = women_data)
```
...
men_lm <- ggplot(men_data, aes(x = EA1, y = accu)) +
  ...
  theme_minimal() +
  labs(x = "EA1", y = "Accuracy, %", title = "y = 75.10 + 3.18X, p = 0.019 ") +  # Adding axis labels and title
